...
class MobileConfirm extends Component {
  ...
  handleGotoStepInformationCode() {
    const {
      pack_type, identification_type, identity
    } = this.state;
  ...
  handleGotoStepIdentity() {
    const { pack_type, your_details } = this.state;
    this.props.history.push('/dashboard/mobile/identity', {
      pack_type,
      your_details
    });
  }
  ...
  render() {
    const {
      your_details, agree_terms, identification_type, identity
    } = this.state;
    return (
      <Section className="main-section bg-maze">
        <Container>
          ...
          <div className="eat-container">
            <Step steps={['Choose plan', 'Your Details', 'Identity', 'Confirm']} index={4} />
            <Modal
